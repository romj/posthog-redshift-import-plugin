...
// RedshiftImportPlugin : 2 properties --> global and config
...
//previously : const REDIS_OFFSET_KEY = 'import_offset'
...
//new properties for RedshiftImportPlugin ? (3rd one)
...
However it might only be method achieved in order to verify that everything works
*/
...
const getTotalRowsToImport = async (config) => {
    ...
    console.log('output : ', totalRowsResult.queryResult.rows[0].count)
...
const importAndIngestEvents = async (
    ...
    //check if number of tries > 15 or 
    ...
    const values = [offset]
    // whas is the role of values argument?
